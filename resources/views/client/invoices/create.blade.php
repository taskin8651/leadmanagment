...
<x-page-header title="Create Invoice" subtitle="{{ $lead ? 'Billing ' . $lead->name : 'Create a new invoice' }}">
    <x-slot:actions><a href="{{ route('client.invoices.index') }}" class="btn btn-light"><i class="bi bi-arrow-left me-1"></i>Back to Invoices</a></x-slot:actions>
</x-page-header>

<form method="POST" action="{{ route('client.invoices.store') }}" id="invoiceForm">
    @csrf
    ...
    <div class="card p-4 form-section fade-up mb-3">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input class="form-control" name="customer_name" value="{{ old('customer_name', $lead->name ?? '') }}" required>
            </div>
            <div class="col-md-4">
                <label class="form-label">Phone</label>
                <input class="form-control" name="customer_phone" value="{{ old('customer_phone', $lead->phone ?? '') }}">
            </div>
            <div class="col-md-4">
                <label class="form-label">Email</label>
                <input class="form-control" type="email" name="customer_email" value="{{ old('customer_email', $lead->email ?? '') }}">
            </div>
            <div class="col-12">
                <label class="form-label">Address</label>
                <textarea class="form-control" name="customer_address" rows="2">{{ old('customer_address', $lead->company_name ?? '') }}</textarea>
            </div>
            <div class="col-md-4">
                <label class="form-label">Customer GSTIN</label>
                <input class="form-control" name="customer_gstin" value="{{ old('customer_gstin') }}" placeholder="22AAAAA0000A1Z5">
            </div>
            <div class="col-md-4">
                <label class="form-label">Place of Supply</label>
                <input class="form-control" name="place_of_supply" value="{{ old('place_of_supply') }}" placeholder="e.g. Maharashtra">
            </div>
            <div class="col-md-4 d-flex align-items-end">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="is_interstate" value="1" id="isInterstate" @checked(old('is_interstate'))>
    ...
    <div class="card p-4 form-section fade-up mb-3">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input class="form-control" type="date" name="issue_date" value="{{ old('issue_date', now()->format('Y-m-d')) }}" required>
            </div>
            <div class="col-md-4">
        ...
        <div class="row g-3 mt-1 justify-content-end">
            <div class="col-md-5">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <input class="form-control" style="max-width:150px" type="number" step="0.01" min="0" name="discount" id="discountInput" value="{{ old('discount', 0) }}">
                </div>
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <label class="form-label mb-0">Tax (%)</label>
                    <input class="form-control" style="max-width:150px" type="number" step="0.01" min="0" max="100" name="tax_percent" id="taxInput" value="{{ old('tax_percent', 0) }}">
                </div>
                <hr>
    ...
    <div class="card p-4 form-section fade-up mb-3">
        ...
        <textarea class="form-control" name="notes" rows="2" placeholder="Payment terms, thank-you note, etc.">{{ old('notes') }}</textarea>
    </div>

    <div class="d-flex gap-2">
        <button class="btn btn-primary"><i class="bi bi-check2 me-1"></i>Create Invoice</button>
        <a href="{{ route('client.invoices.index') }}" class="btn btn-light">Cancel</a>
    </div>
</form>
